...
const admissionValidation = [
    body('studentName').trim().isLength({ min: 2, max: 50 }).matches(/^[a-zA-Z\s]+$/),
    body('parentName').trim().isLength({ min: 2, max: 50 }).matches(/^[a-zA-Z\s]+$/),
    body('email').isEmail().normalizeEmail(),
    body('phone').isMobilePhone(),
    body('grade').isIn(['Nursery', 'LKG', 'UKG', '1st', '2nd', '3rd', '4th', '5th', '6th', '7th', '8th', '9th', '10th']),
    body('previousSchool').optional().trim().isLength({ max: 100 })
];
